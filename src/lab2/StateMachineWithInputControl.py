...
def accepts(exception):
    def arg_type(f):
        @wraps(f)
        def new_f(*args):
            if type(args[1])!=str:
               raise exception("positional argument 1 should be a string")
            return f(*args)
        return new_f

    return arg_type
def accepts_add_state(exception):
    def arg_type(f):
        @wraps(f)
        def new_f(*args):
            if type(args[1])!=str:
               raise exception("positional argument 1 should be a string")
            if type(args[2])!=dict:
               raise exception("positional argument 2 should be a type of dict")
            return f(*args)
        return new_f

    return arg_type

def accepts_add_transition(exception):
    def arg_type(f):
        @wraps(f)
        def new_f(*args):
            if type(args[1])!=str:
               raise exception("positional argument 1 should be a string")
            if type(args[2])!=str:
               raise exception("positional argument 2 should be a string")
            if type(args[3])!=str:
               raise exception("positional argument 3 should be a string")
            if not hasattr(args[4],"__call__"):
                raise exception("positional argument 5 should be a function")
            if not hasattr(args[5],"__call__"):
                raise exception("positional argument 6 should be a function")
...
def accepts_set_transition(f):
    ...
    def new_f(*args):
        if type(args[1])!=Transition:
            raise Exception("positional argument 1 should be a type of Transition")
        return f(*args)
    return new_f
...
def accepts_activate(f):
    ...
    def new_f(*args):
        if type(args[1])!=int:
            raise Exception("positional argument 1 should be a type of int")
        return f(*args)

    return new_f

def accepts_execute(exception):
    def arg_type(f):
        @wraps(f)
        def new_f(*args):
            #print(args)
            if type(args[1])!=str:
               raise exception("positional argument 1 should be a string")
            if type(args[2])!=int:
               raise exception("positional argument 2 should be a type of int ")
            return f(*args)
        return new_f
...
class FiniteStateMachine(object):
    ...
    @accepts_add_state(Exception)
    def add_state(self, name, light):
        state = State(name, light)
        self.states.append(state)

    @accepts_add_transition(Exception)
